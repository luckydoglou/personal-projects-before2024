...
int main(void){
  unsigned long i, last, now, SW1, SW2;
  ...
  while(1){
    ...
    SW1 = GPIO_PORTF_DATA_R & 0x10; // read the SW1(PF4) bit
    SW2 = GPIO_PORTF_DATA_R & 0x01; // read the SW2(PF0) bit
    
    if (SW1 != 0x10 || SW2 != 0x01) { // if either SW1(PF4) or SW2(PF0) is pressed
      Led = Led^0x02;            // toggle red LED
    } else { // if both switches are not pressed
      Led = 0x00; // keep PF1(LED) off
    }
    GPIO_PORTF_DATA_R = Led;   // output 
    if(i<50){
      now = NVIC_ST_CURRENT_R;
      Time[i] = (last-now) & 0x00FFFFFF;  // 24-bit time difference
      if (Data[i - 1] != (GPIO_PORTF_DATA_R & 0x13)) { // make sure not record the same data
        Data[i] = GPIO_PORTF_DATA_R & 0x13; // record PF4(SW1), PF0(SW2), PF1(LED)
