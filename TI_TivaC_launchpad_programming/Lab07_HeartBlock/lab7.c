...
void WaitForASLow(void){
	...
	do {
		SW1 = GPIO_PORTF_DATA_R & 0x10; // input from PF4
	} while (SW1 != 0x00); // while SW1 is not pressed, continue looping
...
void WaitForASHigh(void){
	...
	do {
		SW1 = GPIO_PORTF_DATA_R & 0x10; // input from PF4
	} while (SW1 == 0x00); // while SW1 is pressed, continue looping
...
void SetVT(void){
	...
	GPIO_PORTF_DATA_R = GPIO_PORTF_DATA_R | 0x02; // set PF1(bit 1) to high(1) 
...
void ClearVT(void){
	...
	GPIO_PORTF_DATA_R = GPIO_PORTF_DATA_R & (~0x02); // set PF1(bit 1) to low(0)
...
void SetReady(void){
	...
	GPIO_PORTF_DATA_R = GPIO_PORTF_DATA_R | 0x08; // set PF3(bit 3) to high(1)
...
void ClearReady(void){
	...
	GPIO_PORTF_DATA_R = GPIO_PORTF_DATA_R & (~0x08); // set PF3(bit 3) to low(0)
...
void Delay1ms(unsigned long msec){
	unsigned long count;
	while (msec > 0) {
		count = 12705; // this is magic! Different when use simulator and real board...
		while (count > 0) {
			count--;
		}
		msec--;
